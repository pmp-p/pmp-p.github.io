...
def status(state):
    ...
    try:
        if self.last_state != state:
            print(self.__class__.__name__, " state:", self.last_state, "->", state)
...
def load_sound(filename):
    ...
    return pygame.mixer.Sound(
        os.path.join(os.path.join(os.getcwd(), "sounds", filename))
    )
...
class Spaceship(GameObject):
    ...
    def draw_on(self, screen):
        ...
        if self.is_throttle_on:
            ...
            new_image, rect = rotate_center(
                self.image, self.image.get_rect(), self.angle
            )
...
class Rock(GameObject):
    ...
    def __init__(self, position, size, speed=2):
...
class MyGame(object):
    ...
    PLAYING, DYING, GAME_OVER, STARTING, WELCOME = "PLAYING, DYING, GAME_OVER, STARTING, WELCOME".split(', ')
#range(5)
    ...
    def init(self):
        ...
        self.soundtrack = load_sound("soundtrack.ogg")
        self.soundtrack.set_volume(0.3)

        # loading the dying, game over and missile sounds
        self.die_sound = load_sound("die.ogg")
        self.gameover_sound = load_sound("game_over.ogg")
        self.missile_sound = load_sound("fire.ogg")
    ...
    def step(self):
        ...
        for event in pygame.event.get():
            ...
            # switch from game over screen to new game
                ...
                status( MyGame.STARTING )

            # user is clicking to start a new game
            elif event.type == pygame.MOUSEBUTTONDOWN and (
                self.state == MyGame.STARTING or self.state == MyGame.WELCOME
            ):
                self.do_init()
                ...
                for rock in self.rocks:
                    if rock.size == "big":
                        ...
                        if distance(missile.position, rock.position) < 80:
                            ...
                            self.make_rock(
                                "normal", (rock.position[0] + 10, rock.position[1])
                            )
                            self.make_rock(
                                "normal", (rock.position[0] - 10, rock.position[1])
                            )
                        ...
                        if distance(missile.position, rock.position) < 55:
                            ...
                            if missile in self.spaceship.active_missiles:
                                self.spaceship.active_missiles.remove(missile)
                            self.make_rock(
                                "small", (rock.position[0] + 10, rock.position[1])
                            )
                            self.make_rock(
                                "small", (rock.position[0] - 10, rock.position[1])
                            )
    ...
    def rocks_physics(self):
        ...
        if len(self.rocks) > 0:
            ...
            for rock in self.rocks:
                ...
                if (
                    distance(rock.position, self.spaceship.position)
                ...
                # 10 rocks on the screen, create a new rock with the same size
                    ...
                    rock.position, (self.width / 2, self.height / 2)
