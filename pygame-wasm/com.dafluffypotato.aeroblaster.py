#!https://dafluffypotato.itch.io/aeroblaster
size = 0
# Setup Python ----------------------------------------------- #
        ...
        if final_tile_map[tile][0] == 16:
            ...
            entity_dat = [
                e.entity(
                    ...
                    entity_info[final_tile_map[tile][0]][2],
                ),
                False,
            ]
...
# while True:
def loop():
    global size, mainClock, screen, display, main_display, tileset_images, gun_img, cursor_img, turret_barrel_img, turret_shot_img, core_img, turret_example_img, shot_example_img, controls_1, controls_2, jump_s, shoot_s, turret_shoot_s, point_s, explosion_s, death_s, font_dat, font, entity_info, top_tile_list, level, tile_map, entities, map_height, spawnpoint, total_cores, limits, current_fps, dt, last_frame, game_speed, player, player_grav, jumps, player_speed, air_timer, last_movement, shoot_timer, dead, total_time, level_time, win, moved, player_velocity, bullets, explosion_particles, particles, circle_effects, flashes, camera_sources, bar_height, left, right, click, background_timer, scroll_target, scroll, true_scroll, controls_timer, shoot_s_cooldown, tracer
    # Background --------------------------------------------- #
    ...
    for i in range(16):
        ...
        pygame.draw.line(
            display,
            (8, 5, 8),
            (0, i * 20 - background_timer),
            (
                display.get_width(),
                i * 20 - background_timer + 30 * (1.2 / (game_speed + 0.2)),
            ),
            7,
        )
    ...
    true_scroll[0] += int(
        (scroll_target[0] - display.get_width() / 2 - true_scroll[0]) / 10 * dtf(dt)
    )
    true_scroll[1] += int(
        (scroll_target[1] - display.get_height() / 2 - true_scroll[1]) / 10 * dtf(dt)
    )
    ...
    for tile in tile_map:
        ...
        main_display.blit(
            tileset_images[tile_map[tile][0]],
            (tile_x * 16 - scroll[0], tile_y * 16 - scroll[1]),
        )
    ...
    for tile in top_tiles:
        ...
        if (target_right in tile_map) and (tile_map[target_right][0] < 9):
            pygame.draw.line(
                main_display,
                (74, 57, 79),
                (tile[0] * 16 + 16 - scroll[0], tile_y * 16 + 1 - scroll[1]),
    ...
    for i, circle in sorted(list(enumerate(circle_effects)), reverse=True):
        ...
        if circle[2] < 1:
            ...
            pygame.draw.circle(
                main_display,
                circle[5],
                (circle[0][0] - scroll[0], circle[0][1] - scroll[1]),
                int(circle[1]),
    ...
    for i, entity in sorted(list(enumerate(entities)), reverse=True):
        ...
        if entity[0].type == "turret":
                ...
                if moved:
                    ...
                    bullets.append(
                        [
                            "turret",
                            [
                                entity[0].get_center()[0] + math.cos(rot) * 10,
                                entity[0].get_center()[1] + 10 + math.sin(rot) * 10,
                            ],
                            rot,
                            projectile_speed,
                        ]
                        ...
                        flashes.append(
                            [
                                [
                                    ...
                                    + math.cos(rot + math.radians(rot_offset)) * 6,
                                    entity[0].get_center()[1]
                                    + 10
                                    + math.sin(rot + math.radians(rot_offset)) * 6,
                                ],
            ...
            e.blit_center(
                main_display,
                pygame.transform.rotate(turret_barrel_img, -entity[2]),
                [
                    entity[0].get_center()[0] - scroll[0],
                    entity[0].get_center()[1] - scroll[1] + 10,
                ],
            )
        if (entity[0].type == "core") and (entity[0].animation_frame > 40):
            if entity[0].animation_frame == 41:
                for i2 in range(5):
                    ...
                    particles.append(
                        e.particle(
                            entity[0].x + random.randint(0, entity[0].size_x),
                            entity[0].y + 3,
                            "p",
                            [math.cos(rot) * speed, math.sin(rot) * speed],
                            0.07,
                            random.randint(20, 35) / 10,
                            (92, 163, 135),
                        )
                    )
            outline(
                entity[0].get_current_img(),
                main_display,
                [entity[0].x - scroll[0], entity[0].y - scroll[1]],
                (254, 254, 254),
            )
        if (
            (entity[0].type == "core")
            and (entity[0].animation_frame <= 40)
            and (entity[0].animation_frame > 10)
        ):
            outline(
                entity[0].get_current_img(),
                main_display,
        ...
        if entity[1]:
            if entity[0].type == "core":
                circle_effects.append(
                    [entity[0].get_center(), 10, 4, 5, 0.2, (92, 163, 135)]
                )
                circle_effects.append(
                    [entity[0].get_center(), 0, 7, 4, 0.1, (92, 163, 135)]
                )
                for i2 in range(30):
                    rot = math.radians(-random.randint(20, 160))
            ...
            if entity[0].type == "turret":
                for i2 in range(40):
                    ...
                    particles.append(
                        e.particle(
                            entity[0].x + random.randint(0, entity[0].size_x),
                            entity[0].y + random.randint(0, entity[0].size_y),
                            "p",
                            [math.cos(rot) * speed, math.sin(rot) * speed],
                            0.03,
                            random.randint(10, 35) / 10,
                            (79, 66, 113),
                        )
                    )
            entities.pop(i)
    ...
    player_velocity = [
        normalize(player_velocity[0], 0.15 * dtf(dt) * game_speed),
    ...
    x = player.x
    y = player.y
    collisions = {"bottom": False, "top": False, "left": False, "right": False}
    if not dead:
        if (
            (player.x > scroll[0])
            and (player.x < scroll[0] + display.get_width())
            and (player.y > scroll[1])
            and (player.y < scroll[1] + display.get_height())
        ):
    ...
    rot = -math.degrees(
        math.atan2(
            my - (int(player.get_center()[1]) - scroll[1]),
            mx - (int(player.get_center()[0]) - scroll[0]),
        )
    )
    if not dead:
        e.blit_center(
            main_display,
            pygame.transform.rotate(e.flip(gun_img, False, flip), rot),
            [
                int(player.get_center()[0]) - scroll[0],
                int(player.get_center()[1]) - scroll[1] + 3,
            ],
        )
    ...
    if click and not dead:
        if shoot_timer == 0:
            ...
            bullets.append(
                [
                    "player",
                    [player.get_center()[0], player.get_center()[1] + 3],
                    math.radians(-rot + random.randint(0, 12) - 6),
                    20,
                ]
            )
            for i in range(3):
                ...
                flashes.append(
                    [
                        [
                            player.get_center()[0]
                            + math.cos(math.radians(-rot + rot_offset)) * 4,
                            player.get_center()[1]
                            + math.sin(math.radians(-rot + rot_offset)) * 4,
                        ],
                        random.randint(10, 30),
                        math.radians(-rot + rot_offset),
                        8,
                        random.randint(0, 8),
                    ]
                )
    ...
    for i, bullet in sorted(list(enumerate(bullets)), reverse=True):
        ...
        if bullet[0] == "turret":
            shot_img = pygame.transform.rotate(
                turret_shot_img, -math.degrees(bullet[2])
            )
            outline(
                shot_img,
                main_display,
                (
                    bullet[1][0] - scroll[0] - int(shot_img.get_width() / 2),
                ...
                if player.obj.rect.collidepoint(bullet[1]):
                    if not dead and not win:
                        ...
                        for i2 in range(60):
                            ...
                            particles.append(
                                e.particle(
                                    player.x + random.randint(0, player.size_x),
                                    player.y + random.randint(0, player.size_y),
                                    "p",
                                    [math.cos(rot) * speed, math.sin(rot) * speed],
                                    0.03,
                                    random.randint(10, 35) / 10,
        ...
        if bullet[0] == "player":  # type, pos, angle, speed
            ...
            for i2 in range(dis):
                pos = [
                    int((bullet[1][0] - math.cos(bullet[2]) * (dis - i2)) / 16),
                    int((bullet[1][1] - math.sin(bullet[2]) * (dis - i2)) / 16)
                    % map_height,
                ]
                if xy2str(pos) in tile_map:
                    if tile_map[xy2str(pos)][0] < 14:
                        for i3 in range(12):
                            ...
                            pos2 = [
                                bullet[1][0] - math.cos(bullet[2]) * (dis - i2),
                                bullet[1][1] - math.sin(bullet[2]) * (dis - i2),
                            ]
                            bullet_v = [
                                math.cos(bullet[2]) * bullet[3] / 6,
                                math.sin(bullet[2]) * bullet[3] / 6,
                            ]
                            explosion_particles.append(
                                [
                                    ...
                                    pos2.copy(),
                                    [
                                        math.cos(math.radians(rot)) * speed
                                        - bullet_v[0],
                            ...
                            flashes.append(
                                [
                                    [
                                        bullet[1][0] + math.cos(math.radians(rot)) * 4,
                                        bullet[1][1] + math.sin(math.radians(rot)) * 4,
                                    ],
        ...
        if not popped:
            if (abs(bullet[1][0] - player.get_center()[0]) > 300) or (
                abs(bullet[1][1] - player.get_center()[1]) > 300
            ):
                bullets.pop(i)
    ...
    for i, explosion_particle in sorted(
        list(enumerate(explosion_particles)), reverse=True
    ):
        if explosion_particle[0] == "3rd":
            size = (
                (explosion_particle[4][1] - explosion_particle[4][0])
                / explosion_particle[4][1]
            ...
            if explosion_particle[4][0] > 2:
                pygame.draw.circle(
                    ...
                    (141, 137, 163),
                    (
                        explosion_particle[1][0] - scroll[0],
                        explosion_particle[1][1] - scroll[1],
                    ),
                    size,
                )
        if explosion_particle[0] == "core":
            ...
            pos = [
                int(explosion_particle[1][0] / 16),
                int(explosion_particle[1][1] / 16) % map_height,
            ]
            if (xy2str(pos) in tile_map) and (tile_map[xy2str(pos)][0] < 14):
                explosion_particle[2][0] = -explosion_particle[2][0] * 0.7
                explosion_particle[1][0] += (
                    explosion_particle[2][0] * dtf(dt) * game_speed * 3
                )
            ...
            if (xy2str(pos) in tile_map) and (tile_map[xy2str(pos)][0] < 14):
                ...
                explosion_particle[1][1] += (
                    explosion_particle[2][1] * dtf(dt) * game_speed * 3
                )
            ...
            if random.randint(1, 2) == 1:
                explosion_particles.append(
                    ["3rd", explosion_particle[1].copy(), [0, 0], size + 1, [0, 16]]
                )
            if explosion_particle[4][0] < 12:
                if random.randint(1, 3) == 1:
                    explosion_particles.append(
                        [
                            "2nd",
                            explosion_particle[1].copy(),
                            [(random.randint(0, 20) / 10 - 1), 0],
                            random.randint(8, 16),
                            [
                                0,
                                (
        ...
        if explosion_particle[0] == "2nd":
            ...
            color_offset = (
                (explosion_particle[4][0] / explosion_particle[4][1])
                * 30
                * (1 + explosion_particle[3] / 8)
            )
            pygame.draw.circle(
                main_display,
                (
                    255 - color_offset * 1.1,
                    255 - color_offset * 1.2,
                    255 - color_offset,
                ),
                (
                    explosion_particle[1][0] - scroll[0],
    ...
    if level == 1:
        ...
        text.show_text(
            "shoot",
            150 - int(get_text_width("shoot", 1) / 2),
            35,
            1,
            9999,
            font,
    ...
    if level == 3:
        text.show_text(
            "shoot    avoid",
            150 - int(get_text_width("shoot    avoid", 1) / 2),
            45,
            1,
            9999,
            font,
    ...
    if win == 0:
        screen.blit(
            pygame.transform.scale(
                display, (900 + int(bar_height * 3), 600 + int(bar_height * 3))
            ),
    ...
    # UI ----------------------------------------------------- #
    text.show_text(
        convert_time(total_time) + " > " + convert_time(level_time),
        3,
        3,
        1,
        9999,
    ...
    text.show_text(
        str(total_cores - cores_left) + "/" + str(total_cores),
        16,
        23,
        1,
        9999,
    ...
    #aio.steps.remove(step)
    #aio.steps.append(step_end)
    return True
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE:
                ...
                sys.exit()
    text.show_text(
        "You Win!", 150 - get_text_width("You Win!", 1) / 2, 90, 1, 9999, font, display
    )
    text.show_text(
        convert_time(total_time),
        150 - get_text_width(convert_time(total_time), 1) / 2,
        100,
        1,
        9999,
        font,
...
if __EMSCRIPTEN__:
    ...
    aio.steps.append(pygame.display.update)
else:
    import os, trace
    tracer = trace.Trace(count=False, trace=True)
    #loop() # with the # trace will break

    """
 --- modulename: com.dafluffypotato.aeroblaster, funcname: dtf
com.dafluffypotato.aeroblaster.py(194):     return dt / 1000 * 60
com.dafluffypotato.aeroblaster.py(719):     if click and not dead:
com.dafluffypotato.aeroblaster.py(752):     for i, bullet in sorted(list(enumerate(bullets)), reverse=True):
com.dafluffypotato.aeroblaster.py(883):     for i, particle in sorted(list(enumerate(particles)), reverse=True):
com.dafluffypotato.aeroblaster.py(905):     for i, flash in sorted(list(enumerate(flashes)), reverse=True):
com.dafluffypotato.aeroblaster.py(937):     for i, explosion_particle in sorted(
com.dafluffypotato.aeroblaster.py(938):         list(enumerate(explosion_particles)), reverse=True
com.dafluffypotato.aeroblaster.py(937):     for i, explosion_particle in sorted(
com.dafluffypotato.aeroblaster.py(1050):     e.blit_center(main_display, cursor_img, (mx, my))
Traceback (most recent call last):
  File "/data/git/python-wasm-plus/demos/com.dafluffypotato.aeroblaster/static/com.dafluffypotato.aeroblaster.py", line 1319, in <module>
    tracer.runfunc(loop)
    ^^^^^^^^^^^^^^^^^^^^
  File "/data/git/python-wasm-plus/devices/x86_64/usr/lib/python3.11/trace.py", line 461, in runfunc
    result = func(*args, **kw)
             ^^^^^^^^^^^^^^^^^
  File "/data/git/python-wasm-plus/demos/com.dafluffypotato.aeroblaster/static/com.dafluffypotato.aeroblaster.py", line 1050, in loop
    e.blit_center(main_display, cursor_img, (mx, my))
                                ^^^^^^^^^^
NameError: name 'get_ticks' is not defined
"""
    os.system('reset')
    print("tracing...")
    tracer.runfunc(loop)
