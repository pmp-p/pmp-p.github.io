...
async function window_load(event) {
    ...
    for (const script of document.getElementsByTagName('script')) {
        if (script.type == 'module') {
            if ( (script.src.search('#')>0) && ( script.src.search('pythons') >0) ) {
...
// TODO scripts argv ( sys.argv )
    ...
    if (vm.script.interpreter.startsWith("python")) {
        console.log("VM.sti : no preemption (cpython)")
        vm.script.sti = false
        //window.cpython = true
        //vm.script.stdin_raw = false;
        //vm.script.stdin_echo = true
        window.stdin_flush = false;

    } else {
        window.cpython = false
    }
