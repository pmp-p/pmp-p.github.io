...
const modularized = (typeof python310 != 'undefined')
...
function pythonvm(vterm, config) {
    ...
    const Module = {
        ...
        locateFile : function(path, prefix) {
            if (path == "main.data") {
                const url = (config.cdn || "" )+`python310/${path}`
    ...
    if (modularized) {
        python310(Module).then( async (vm) => {
                VM = vm
        ...
        jswasmloader.setAttribute("src", (config.cdn || "")+ "python310/main.js")
