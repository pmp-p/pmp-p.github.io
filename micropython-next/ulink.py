...
class CallPath(dict):
    ...
    def __getattr__(self, name):
        ...
        if DBG:
            if len(self.__csp):
                print("17x: cp-ast {} with PARENT call(s) pending".format((fqn)), *self.__csp)
            else:
                print("17x: cp-ast {}".format((fqn)))
...
#            return
    ...
    def _cp_setup(self, key, v):
        if v is None:
            if DBG:
                print("126: cp-set {}.{}, NOREF => left unset".format((self.__fqn), (key)))
            return


        elif DBG:
            if len(self.__csp):
                print("122: cp-set {}.{} <= {} with call(s) PARENT pending".format((self.__fqn), (key), (v)), *self.__csp)
            else:
                print("122: cp-set {}.{} <= {}".format((self.__fqn), (key), (v)), *self.__csp)

        if not isinstance(v, self.__class__):
            print("130: cp-set {} := {} ".format((key), (v)))
...
class SyncProxy(Proxy):
    ...
    def set(self, cp, argv, cs=None):
        ...
        if cs is not None:
            ...
            jsdata = "JSON.parse(`{}`)".format((dumps(targv)))
            target = solvepath.rsplit('.', 1)[0]
            assign = 'JSON.parse(`{}`)'.format((dumps(argv)))
    ...
    async def wait_answer(self, cid, fqn, solvepath):
        ...
        while tmout > 0:
            ...
            print("TIMEOUT({} ms): wait_answer({}) for {}".format((self.tmout), (cid), (fqn)))
