...
// window.plink  will hold embed I/O , stdin and stdout buffers via plink.0xx.js
...
// separated file plink_xxx.js : entry point is embed_call(struct_from_json)
...
function init_repl_begin(){
    ...
    for(var i = 0; i < scripts.length; i++){
        ...
        if(script.type == window.script_type_python){
            if (script.text)
...
if ( window.cpython ) {
    ...
    window.script_interpreter = "wapy"
}
...
async function pythons(argc, argv){
    ...
    for(var i = 0; i < scripts.length; i++){
        ...
        if(script.type == script_type_python){
            console.log('requesting script interpreter : ' + script_interpreter )
